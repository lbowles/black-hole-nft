...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const voidableBlackHoles1 = await deployments.get("VoidableBlackHoles")
  ...
  await deploy("BlackHolesV2", {
    ...
    args: [name, symbol, mergeOpenTimestamp, renderer.address, blackHoles.address, voidableBlackHoles1.address],
    autoMine: true, // speed up deployment on local network (ganache, hardhat), no effect on live networks
    // gasPrice: ethers.utils.parseUnits("37", "gwei"),
  })
}
export default func
func.tags = ["BlackHolesV2"]
func.dependencies = ["VoidableBlackHoles", "BlackHoles", "Libraries", "Renderer"]
